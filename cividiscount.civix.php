 ...
 * @return array(string)
 */
function _cividiscount_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_cividiscount_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
 ...
 */
function _cividiscount_civix_civicrm_themes(&$themes) {
  $files = _cividiscount_civix_glob(__DIR__ . '/*.theme.php');
  foreach ($files as $file) {
 ...
 */
function _cividiscount_civix_glob($pattern) {
  $result = glob($pattern);
...
function _cividiscount_civix_civicrm_entityTypes(&$entityTypes) {
